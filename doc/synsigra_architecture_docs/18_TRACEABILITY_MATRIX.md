...
| `TRC-FMT-WFDB-TOOL-001` | Native WFDB conformance follow-up | Native WFDB tooling conformance | `teszt/wfdb_native_conformance.cmake` | `TEST-WFDB-NATIVE-001` | [signal_synth#55](https://github.com/tamask1s/signal_synth/issues/55) |
| `TRC-FMT-EDF-TOOL-001` | Native EDF/BDF conformance follow-up | Native EDF/BDF reader conformance | `src/ecg_edf_bdf_export.*`, `teszt/edf_bdf_native_conformance.py` | `TEST-EDF-BDF-NATIVE-001`, `TEST-EDF-BDF-EXPORT-001` | [signal_synth#56](https://github.com/tamask1s/signal_synth/issues/56) |
| `TRC-CI-RUNTIME-001` | Verification workflow policy | Linux full default CI and manual Windows smoke | `.github/workflows/verification.yml`, `WAY_OF_WORKING.md`, `doc/synsigra_architecture_docs/17_TRACEABILITY_SOP.md` | `CI-VER-001` | [signal_synth#57](https://github.com/tamask1s/signal_synth/issues/57) |
| `TRC-SAAS-CHAL-001` | `REQ-CHAL-003..009`, `REQ-PKG-001..005`, `REQ-PY-001..008`, `REQ-NFR-002..008` | SaaS challenge package assembly | `src/challenge_assembly.*`, `src/challenge_package.*`, `apps/signal_synth_cli/`, `python/synsigra/` | `TEST-CHALLENGE-PACKAGE-001`, `TEST-CLI-001`, `TEST-PYTHON-SCORING-001`, `TEST-BUILD-001` | [signal_synth#58](https://github.com/tamask1s/signal_synth/issues/58) |
...
| `TRC-HRV-SCORE-001` | `REQ-HRV-SCORE-001..004` | HRV scoring through the generic measurement contract | `src/measurement_io.*`, `src/measurement_scoring.*`, `apps/signal_synth_cli/`, `python/synsigra/measurements.py`, `python/synsigra/local_verify.py` | `TEST-HRV-GENERIC-001`, `TEST-MEASUREMENT-PYTHON-001`, `TEST-CLI-001`, `TEST-PYTHON-SCORING-001`, `TEST-BUILD-001` | [signal_synth#42](https://github.com/tamask1s/signal_synth/issues/42), [signal_synth#93](https://github.com/tamask1s/signal_synth/issues/93) |
| `TRC-HRV-PACK-001` | `REQ-HRV-SCN-001..006`, `REQ-HRV-GT-001..004`, `REQ-HRV-MET-001..005`, `REQ-HRV-SCORE-001..004` | HRV robustness v2, controlled VLF and pipeline diagnostics | `src/clinical_ecg.*`, `src/ecg_scenario.*`, `src/ecg_scenario_json.*`, `src/hrv_metrics.*`, `src/measurement_scoring.*`, `examples/packs/hrv_robustness_v2*`, `examples/scenarios/hrv/` | `TEST-HRV-PACK-001`, `TEST-ECG-PACK-001`, `TEST-HRV-METRICS-001`, `TEST-HRV-GENERIC-001`, `TEST-PYTHON-SCORING-001` | [signal_synth#89](https://github.com/tamask1s/signal_synth/issues/89) |
...
| `TRC-CORE-CLOSE-001` | `CORECLOSE-REQ-001..010`, supporting `REQ-CHAL-003..009`, `REQ-PY-001..008`, `REQ-NFR-002..008` | Core contract hardening and SaaS handoff closure | `src/ecg_pack.*`, `src/challenge_package.*`, `src/challenge_assembly.*`, `apps/signal_synth_cli/`, `python/synsigra/`, `examples/catalog/`, `examples/packs/*expectations.json` | `TEST-ECG-PACK-001`, `TEST-CHALLENGE-PACKAGE-001`, `TEST-CHALLENGE-PYTHON-SECURITY-001`, `TEST-PYTHON-SCORING-001`, `TEST-RR-QTC-PACK-001`, `TEST-BUILD-001`, `TEST-INTEGRATION-CONTRACT-001`, `TEST-PACK-METADATA-EXPORT-001` | [signal_synth#91](https://github.com/tamask1s/signal_synth/issues/91) |
...
| `TRC-VER-PROTOCOL-002` | `PROTO2-REQ-001..006` | Authoritative packaged protocol and evidence/diagnostic execution modes | `python/synsigra/challenge.py`, `python/synsigra/local_verify.py`, `examples/packs/*expectations.json` | `TEST-PROTOCOL-V2-001`, `TEST-EVIDENCE-MODE-001`, `TEST-INTEGRATION-CONTRACT-001`, `TEST-RR-QTC-PACK-001` | [signal_synth#92](https://github.com/tamask1s/signal_synth/issues/92) |
| `TRC-MEASUREMENT-002` | `MEAS2-REQ-001..006` | Window- and method-identified generic measurement contract v2 | `src/measurement_io.*`, `src/measurement_scoring.*`, `apps/signal_synth_cli/`, `python/synsigra/measurements.py`, `python/synsigra/local_verify.py` | `TEST-MEASUREMENT-V2-001`, `TEST-PY-MEASUREMENT-V2-001`, `TEST-HRV-GENERIC-001`, `TEST-RR-QTC-PACK-001` | [signal_synth#93](https://github.com/tamask1s/signal_synth/issues/93) |
...
| `TEST-CHALLENGE-PACKAGE-001` | `teszt/challenge_package_test.cpp` | Challenge package manifest strict validation, canonicalization, fingerprint, file roles, case file references, content hashes, and assembly API |
| `TEST-CHALLENGE-PYTHON-SECURITY-001` | `teszt/challenge_python_security_test.py` | Strict manifest contract, duplicate-key, role, path, archive-member, symlink, unlisted/missing payload, and integrity rejection at the customer package boundary |
| `TEST-PYTHON-SCORING-001` | `teszt/python_scoring_test.py` | Python challenge directory/archive loading, CLI-generated challenge package loading, waveform access, CSV/JSON detection loading, R-peak/PPG scoring through CLI, and report artifact handling |
...
| `TEST-HRV-GENERIC-001` | `teszt/measurement_scoring_test.cpp` | HRV truth adaptation, window/method identity, metric tolerance scoring, accepted-NN matching, pipeline diagnostics, and generic report contracts |
| `TEST-DELINEATION-IO-001` | `teszt/delineation_io_test.cpp` | Strict identity-free point-event JSON/CSV parsing, deterministic round trips, and malformed-input rejection |
| `TEST-DELINEATION-SCORING-001` | `teszt/delineation_scoring_test.cpp` | Temporal matching, atrial/ventricular anchors, truth evaluability, absent-wave false positives, exclusion windows, and grouped reports |
| `TEST-DELINEATION-PYTHON-001` | `teszt/delineation_python_test.py` | Python/C++ parity, flutter and Mobitz atrial truth, sub-threshold evaluability, and perfect curated-pack verification |
| `TEST-MEASUREMENT-IO-001` / `TEST-MEASUREMENT-V2-001` | `teszt/measurement_io_test.cpp` | Strict measurement-v2 JSON/CSV parsing, canonical round trips, window/method identity, validation, and legacy-input rejection |
| `TEST-MEASUREMENT-SCORING-001` | `teszt/measurement_scoring_test.cpp` | Measurement truth adapters, temporal/index/window matching, circular axes, explicit statuses, context grouping and v2 reports |
| `TEST-MEASUREMENT-PYTHON-001` / `TEST-PY-MEASUREMENT-V2-001` | `teszt/measurement_python_test.py` | Generator-free measurement-v2 verification, legacy rejection, package truth loading, CSV input and exact C++/Python scorer parity |
| `TEST-PROTOCOL-V2-001` | `teszt/protocol_v2_test.py` | Strict protocol shape, exact case-target matrix, profile and stratum coverage, scoring-manifest consistency, fingerprinting, and legacy rejection |
| `TEST-EVIDENCE-MODE-001` | `teszt/rr_qtc_pack_test.py` | Authoritative evidence defaults, forbidden overrides, required-output completeness, diagnostic filtering, and evidence eligibility |
| `TEST-RR-QTC-PACK-001` | `teszt/rr_qtc_pack_test.py` | End-to-end noisy R-peak/RR and QTc pack integrity, RR/QTc truth arithmetic, calibrated 12-lead SNR, protocol-v2 evidence acceptance, and systematic-bias rejection |
...
| `CI-VER-001` | `.github/workflows/verification.yml` | Linux full verification and manual Windows installed-package C++11 smoke |
