...
namespace signal_synth
{
    namespace
    {
        ...
        double hrv_rr_modulation(const clinical_rhythm_config& rhythm, double time_seconds)
        {
            ...
            const double vlf_variance = spectral_variance * rhythm.hrv_vlf_power_fraction;
            const double lf_hf_variance = spectral_variance - vlf_variance;
            const double lf_fraction = rhythm.hrv_lf_hf_ratio / (1.0 + rhythm.hrv_lf_hf_ratio);
            const double lf_variance = lf_hf_variance * lf_fraction;
            const double hf_variance = lf_hf_variance - lf_variance;
            ...
            return hrv_band_component(rhythm.seed, 107, time_seconds, rhythm.hrv_vlf_center_hz, rhythm.hrv_vlf_bandwidth_hz, vlf_variance)
                + hrv_band_component(rhythm.seed, 100, time_seconds, rhythm.hrv_lf_center_hz, rhythm.hrv_lf_bandwidth_hz, lf_variance)
    ...
    clinical_rhythm_config::clinical_rhythm_config()
        : rhythm(clinical_rhythm_sinus), av_conduction(clinical_av_normal), intraventricular_conduction(clinical_iv_normal), preexcitation(clinical_preexcitation_none), pacing_mode(clinical_pacing_ventricular), heart_rate_bpm(60.0), atrial_rate_bpm(75.0), ventricular_escape_rate_bpm(35.0), rr_variability_seconds(0.0), minimum_rr_seconds(0.25), maximum_rr_seconds(3.0), hrv_modulation_enabled(false), hrv_vlf_power_fraction(0.0), hrv_vlf_center_hz(0.02), hrv_vlf_bandwidth_hz(0.004), hrv_lf_hf_ratio(1.0), hrv_lf_center_hz(0.10), hrv_lf_bandwidth_hz(0.04), hrv_hf_center_hz(0.25), hrv_hf_bandwidth_hz(0.12), hrv_respiratory_frequency_hz(0.25), hrv_respiratory_amplitude_seconds(0.0), hrv_respiratory_phase_radians(-1.0), activity_start_seconds(0.0), activity_duration_seconds(0.0), activity_intensity(0.0), first_degree_pr_ms(240.0), mobitz_cycle_length(4), wenckebach_pr_increment_ms(40.0), flutter_conduction_ratio(2), flutter_conduction_pattern(clinical_flutter_fixed), seed(0x434c494e4943414cULL)
    {
    }
