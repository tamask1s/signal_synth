...
namespace
{
    ...
    bool target_compatible(const std::string& target, const signal_synth::ecg_scenario_document& document, std::string& message)
    {
        ...
        if ((target == "rhythm_episode" || target == "rhythm_burden") && document.ecg.rhythm_episode_count() == 0)
        {
            message = "Rhythm episode and burden scoring require at least one configured rhythm episode.";
            return false;
        }
        return true;
    }
...
namespace signal_synth
{
    ...
    std::string scenario_authoring_metadata_json()
    {
        static const field_definition fields[] = {
            ...
            {"$.ecg.rhythm_episodes","Rhythm episodes","episode","ecg_rhythm_episode_array","episode_editor","[]","0","64",0,"items",0,0,true},
            ...
            {"$.hrv.vlf_power_fraction","VLF power fraction","hrv","number","slider","0","0","1","0.01","fraction",0,"{\"path\":\"$.hrv.enabled\",\"equals\":true}",true},
            {"$.hrv.vlf_center_hz","VLF center","hrv","number","number","0.02","0.0033","0.04","0.001","Hz",0,"{\"path\":\"$.hrv.enabled\",\"equals\":true}",true},
            {"$.hrv.vlf_bandwidth_hz","VLF bandwidth","hrv","number","number","0.004","0.000001","0.0734","0.001","Hz",0,"{\"path\":\"$.hrv.enabled\",\"equals\":true}",true},
        ...
        output << "{\"schema_version\":1,\"metadata_version\":" << json_string(metadata_version)
               << ",\"scenario_schema_version\":9,\"supported_scenario_schema_versions\":[2,3,4,5,6,7,8,9],\"groups\":["
        ...
        output << "],\"targets\":["
               ...
               << "{\"name\":\"rhythm_burden\",\"support\":\"local_scoring\",\"requires\":[\"ecg.rhythm_episodes.length>0\"]}],"
               << "\"cross_field_rules\":["
