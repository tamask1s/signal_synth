...
int main()
{
    ...
    ok &= check(annotations && annotations->content.find("{\"schema_version\":2") == 0 && annotations->content.find("\"rr_tachogram\":[") != std::string::npos && annotations->content.find("\"artifact_intervals\":[]") != std::string::npos && annotations->content.find("\"pacing_events\":[]") != std::string::npos && annotations->content.find("\"beat_class\":\"normal\"") != std::string::npos && annotations->content.find("\"r_peak_scoreable\":true") != std::string::npos && annotations->content.find("\"contract\":\"synsigra_observable_event_truth_v1\"") != std::string::npos, "annotation_contract");
    ...
    ok &= check(provenance && provenance->content.find("\"metadata_type\":\"synsigra_export_provenance\"") != std::string::npos && provenance->content.find("\"git_commit\":") != std::string::npos && provenance->content.find("\"package_contract_version\":\"synsigra_challenge_package_v3\"") != std::string::npos && provenance->content.find("\"scoring_manifest_contract_version\":\"synsigra_scoring_manifest_v3\"") != std::string::npos, "provenance_contract");
    ...
    ok &= check(report && report->content.find("Synthetic engineering QA evidence; not diagnosis, nor clinical evidence") != std::string::npos && report->content.find("background:#f3f4f6") != std::string::npos && report->content.find("This report describes synthetic") == std::string::npos && report->content.find("provenance.json") != std::string::npos, "controlled_report_claims");
    ...
    ok &= check(std::string(signal_synth::signal_synth_generator_version()) == "0.10.0-dev", "runtime_generator_version");
    ok &= check(std::string(signal_synth::signal_synth_package_contract_version()) == "synsigra_challenge_package_v3" && std::string(signal_synth::signal_synth_scoring_manifest_contract_version()) == "synsigra_scoring_manifest_v3" && std::string(signal_synth::signal_synth_verification_protocol_contract_version()) == "synsigra_verification_protocol_v2" && std::string(signal_synth::signal_synth_verifier_version()) == "0.15.0", "runtime_contract_versions");
