...
int main()
{
    ...
    ok &= check(signal_synth::resolve_scenario_controls(input, first, first_draws, messages)
        ...
        && first_draws.size() == 2u, "seeded_resolution_is_deterministic");
    ok &= check(first.ecg.heart_rate_bpm() >= 65.0 && first.ecg.heart_rate_bpm() <= 85.0
        && first.ppg.pulse_delay_ms >= 150.0 && first.ppg.pulse_delay_ms <= 230.0, "draws_stay_inside_envelopes");
    ...
    ok &= check(render.parameter_draws.size() == 2u
        && render.resolved_document_identity.document_fingerprint != render.document_identity.document_fingerprint
        && signal_synth::scenario_parameter_draws_json(input, render.resolved_document, render.parameter_draws).find("\"draws\":[{") != std::string::npos, "resolved_scenario_audit_trail");
