...
int main()
{
    ...
    ok &= check(rejects_without_mutation(std::string(input).replace(input.find("\"schema_version\":1"), 18, "\"schema_version\":8"), signal_synth::ecg_json_schema_version), "reject_schema_version");
    ...
    multimodal_roundtrip.ppg.pulse_delay_variation_ms = 10.0;
