...
int main()
{
    ...
    bool ok = true;
    ok &= check(cases.size() == 66 && catalog_ok, "all_66_supported_condition_variants_pass_waveform_quality_gate");
    ...
    ok &= check(signal_synth::ecg_scenario_engine_version() == 8, "morphology_correction_increments_engine_identity");
    return ok ? 0 : 1;
}
